...
struct AIService {
    ...
    func analyzeImages(
        ...
        {
          "name": "物品简短名称（2-4字，如"折叠伞"、"钥匙"、"马克杯"、"数据线"）。命名原则：只保留核心品类名，最多加一个最显著特征词。避免堆砌颜色、材质、品牌等多个修饰词。宁可笼统也不要冗长。",

          "summary": "一句话摘要（10-20字），包含最核心的颜色+品类+最显著特征，如"黑色折叠伞，手柄有磨损"、"白色陶瓷杯，杯口有裂纹"。用于快速浏览，不要与 name 重复。",

          "display_description": "详情页展示用的简短描述（30-60字），用 2-3 句话概括物品最关键的视觉和功能特征。从完整 description 中提炼精华，让用户扫一眼就知道这是什么。不要照搬 description 开头。",

          "display_scene": "详情页展示用的简短场景（20-30字），一句话说清物品放在哪里、什么环境。从完整 scene 中提炼。例如"客厅电视柜第一层抽屉"、"卧室床头柜上"。",

          "location_label": "极简位置标签（2-6字），只用最核心的地点词。例如"家里客厅"、"办公室"、"卧室床头"、"玄关鞋架"。用于详情页标题旁边的位置徽章，展示给用户一眼看到物品在哪。结合用户描述中提到的地点信息，只写地名词，不要动词。",
