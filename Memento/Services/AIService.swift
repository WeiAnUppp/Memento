...
struct AIService {
    ...
    func analyzeImages(
        ...
        你的任务：结合用户的口语描述和多张图片内容，对图片中的主物品做一次【极其详尽、全面】的记录分析。
...

        {
          ...
          "description": "【重点】物品的完整外观描述，至少150字。请像亲眼看到一样写下来，包含以下所有你能观察到的维度：
          ...
          - 遍历画面中所有可见的其它物品，一个一个列出来，用通用中文名词。
          - 至少列出 3-8 个（如果画面中确实有那么多），越多越好。
        ...
        // 用户一句话描述
        let contextText = "用户描述：\(userContext)\n\n请对这些图片（同一物品的不同角度）做极其详尽的记录分析。记住：你看到的所有细节——颜色、材质、形状、品牌logo、周围物品、场景光线、摆放位置——全部写进 description、scene、keywords 和 nearby_objects。不漏掉任何东西。"
